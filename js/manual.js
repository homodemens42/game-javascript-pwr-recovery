...
保護系: RPS／自動動作が原子炉および安全機能を監視します。</pre>
    ...
    {id:'instrumentation',title:'計装系',type:'equipment',equipment:{
      ...
      commands:['STATUS INSTRUMENTATION','READ SG-B-LVL-CH1','READ SG-B-LVL-CH2','READ SG-B-LVL-CH3','CONTROL SG-B-LVL DIAG']
    }},
    {id:'rcs-injection',title:'RCS補給 / 安全注入',type:'equipment',equipment:{
      ...
      commands:['READ RCS','READ PZR','READ RCS-LETDOWN','READ RCS-LETDOWN-ISO','READ SI','CONTROL RCS-LETDOWN-ISO CLOSE']
    }},
    {id:'ccw',title:'補機冷却水系（CCW）',type:'equipment',equipment:{
      ...
      commands:['STATUS SUPPORT','READ CCW-A','READ CCW-B','READ CCW-HEADER','CONTROL CCW-B START']
    }},
    ...
    {id:'glossary-nz',title:'用語集 N–Z',type:'glossary',terms:[
      ...
      ['TIME LIMIT','復旧・解明期限','異常発生後に復旧または異常の解明・必要処置を完了するまでの共通期限です。番号付き全ステージ8時間で、00:00になると時間切れです。'],
    ...
    {id:'cmd-read-support',title:'コマンド — READ / 補助系',type:'commands',filter:(cmd)=>/^READ (TURBINE|GENERATOR|CONDENSER|STEAM-DUMP|ATM-DUMP|OFFSITE|BUS-|EDG-|SG-B-LVL-CH|RCS-LETDOWN|SI$|CCW-)/.test(cmd)},
    ...
    {id:'terminal',title:'ターミナル操作',type:'content',body:()=>`
      ...
      <h2>時間表示</h2><p>ゲーム内時間は現実の1秒をプラント内1分として進行します。TIME LIMITは番号付き全ステージ共通で8時間です。これは異常を復旧・解明するまでの最終期限で、設備状態に関係なく減少します。PLANT TIMEは現在の設備状態からみた物理的な余裕時間を示し、悪化で短く、改善で長くなります。安全側へ収束し物理的な破綻が予測されない場合は --:--、物理的な余裕が残りTIME LIMITより長い場合は &gt; TIME LIMIT と表示されます。</p><h2>練習モード</h2><p>TUTORIALでは画面・調査手順・マニュアルの使い方を案内します。SIMULATIONでは正常運転状態からコマンドを自由に試せます。SIMULATIONにはCLEARとTIME LIMITはなく、RESETで正常初期状態へ戻せます。</p><h2>入力履歴</h2><p>ターミナル入力欄で↑／↓キーを使うと、過去に実行したコマンドを呼び出せます。</p>
