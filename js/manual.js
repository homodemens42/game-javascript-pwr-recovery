...
保護系: RPS／自動動作が原子炉および安全機能を監視します。</pre>
    ...
    {id:'afw',title:'補助給水系（AFW）',type:'equipment',equipment:{
      ...
      commands:['STATUS FEEDWATER','READ AFW-A','READ AFW-B','READ AFW-C','READ AFW-VALVE-A','READ AFW-VALVE-B','READ AFW-VALVE-C']
    }},
    ...
    {id:'electrical',title:'電気系統',type:'equipment',equipment:{
      ...
      commands:['STATUS ELECTRICAL','READ OFFSITE','READ BUS-A','READ BUS-B','READ EDG-A','READ EDG-B','READ EDG-A-BREAKER','READ EDG-B-BREAKER']
    }},
    ...
    {id:'instrumentation',title:'計装系',type:'equipment',equipment:{
      ...
      diagram:`実プロセス値\n        ├→ CHANNEL 1\n        ├→ CHANNEL 2\n        └→ CHANNEL 3\n             ↓\n        チャンネル比較`,
      indications:[['VALUE','表示プロセス値'],['QUALITY','チャンネル有効性表示'],['CHANNEL AGREEMENT','冗長チャンネル間の比較'],['HISTORY','プロセス変化への追従状況'],['DIAGNOSTIC','計装グループの再診断状態']],
      normal:[['冗長チャンネル','通常許容範囲内で概ね一致'],['トレンド','実プロセス変化に追従']],
      notes:['チャンネル値がもっともらしい範囲に留まっていても、実プロセスを追従していない場合があります。','冗長チャンネルや他の物理表示値との比較により、実プロセス変化とセンサー故障を切り分けられます。','チャンネル不一致が継続する場合はDIAGを実行し、計装グループの再診断結果を確認してください。診断結果によって追加の処置が指示される場合があります。'],
      cautions:['独立した表示値と矛盾する単一チャンネルだけから、実際の保有量変化を断定しないでください。'],
      related:[['steam-generator','STEAM GENERATOR'],['protection','PROTECTION SYSTEM']],
      commands:['STATUS INSTRUMENTATION','READ SG-A-LVL-CH1','READ SG-A-LVL-CH2','READ SG-A-LVL-CH3','CONTROL SG-A-LVL DIAG','READ SG-B-LVL-CH1','READ SG-B-LVL-CH2','READ SG-B-LVL-CH3','CONTROL SG-B-LVL DIAG','READ SG-C-LVL-CH1','READ SG-C-LVL-CH2','READ SG-C-LVL-CH3','CONTROL SG-C-LVL DIAG']
    }},
    {id:'rcs-injection',title:'RCS補給 / 安全注入',type:'equipment',equipment:{
      description:'RCSの保有量・圧力が低下した場合、安全注入（SI）が一次系へ補給水を供給します。通常運転中のletdown系統は一次系保有量調整のための抽出経路を持ち、異常時には隔離が必要となる場合があります。',
      diagram:`RCS ─→ LETDOWN ─→ RCS-LETDOWN-ISO ─→ 下流設備\n │\n └← SAFETY INJECTION (SI)`,
      indications:[['RCS PRESSURE','一次系圧力'],['PZR LEVEL / INVENTORY','一次系保有量の指標'],['LETDOWN FLOW','抽出経路の流量'],['SI FLOW','安全注入流量'],['ISOLATION','letdown隔離弁状態']],
      normal:[['RCS圧力','約15.5 MPa'],['PZR水位','中間域'],['SI','通常時は待機'],['LETDOWN隔離弁','通常運転状態に応じた所定位置']],
      notes:['圧力低下と保有量低下が同時に継続する場合は、単なる温度変化ではなく一次冷却材の流出も考慮してください。','安全注入が作動しても、流出量が補給量を上回れば保有量は回復しません。'],
      cautions:['AUTO安全注入の作動は漏えい源の隔離を意味しません。RCS圧力・保有量・補給流量・流出経路を合わせて確認してください。'],
      related:[['reactor-rcs','REACTOR / RCS'],['protection','PROTECTION SYSTEM']],
      commands:['READ RCS','READ PZR','READ RCS-LETDOWN','READ RCS-LETDOWN-ISO','READ SI','CONTROL RCS-LETDOWN-ISO CLOSE','CONTROL RCS-LETDOWN-ISO OPEN']
    }},
    {id:'ccw',title:'補機冷却水系（CCW）',type:'equipment',equipment:{
      description:'補機冷却水系（CCW）は、複数のポンプ・電源設備・安全系補機から熱を回収する共通の支持系統です。主系統喪失時には待機系統を起動して冷却を引き継ぎます。',
      diagram:`CCW-A ─┐\n       ├→ CCW HEADER → RCP補機 / AFW補機 / EDG補機\nCCW-B ─┘`,
      indications:[['PUMP STATE','RUNNING / STANDBY / FAILED'],['HEADER FLOW','共通冷却流量'],['HEADER TEMP','共通冷却水温度'],['DEPENDENT EQUIPMENT TEMP','CCWを必要とする各設備温度']],
      normal:[['CCW-A','RUNNING'],['CCW-B','STANDBY'],['HEADER FLOW','約100 %'],['HEADER TEMP','正常範囲']],
      notes:['無関係に見える複数設備の温度上昇が同時に起きる場合、共通の支持系統を確認してください。','待機ポンプがRUNNINGになっても、ヘッダ流量の回復には時間を要します。'],
      cautions:['共通支持系統の喪失は、個々の安全設備が正常でも時間とともに複数設備を同時に使用不能へ導くことがあります。'],
      related:[['rcp','RCP'],['afw','AUXILIARY FEEDWATER'],['electrical','ELECTRICAL SYSTEM'],['protection','PROTECTION SYSTEM']],
      commands:['STATUS SUPPORT','READ CCW-A','READ CCW-B','READ CCW-HEADER','CONTROL CCW-A START','CONTROL CCW-A STOP','CONTROL CCW-B START','CONTROL CCW-B STOP']
    }},
    ...
    {id:'glossary-nz',title:'用語集 N–Z',type:'glossary',terms:[
      ['PLANT TIME','設備安全余裕時間','現在の設備状態からみた物理的な余裕時間です。状態悪化で短くなり、改善で長くなります。安全側へ収束して物理的な時間制約がなくなった場合は --:-- と表示します。'],
      ['CCW','Component Cooling Water / 補機冷却水','複数の安全系・補機設備から熱を回収する共通支持系統です。'],
      ...
      ['TIME LIMIT','復旧・解明期限','異常発生後に復旧または異常の解明・必要処置を完了するまでの期限です。本施設では8時間を基準とし、00:00になると期限超過です。'],
    ...
    {id:'cmd-read-support',title:'コマンド — READ / 補助系',type:'commands',filter:(cmd)=>/^READ (TURBINE|GENERATOR|CONDENSER|STEAM-DUMP|ATM-DUMP|OFFSITE|BUS-|EDG-|SG-[ABC]-LVL-CH|RCS-LETDOWN|SI$|CCW-)/.test(cmd)},
    ...
    {id:'terminal',title:'ターミナル操作',type:'content',body:()=>`
      ...
      <h2>時間表示</h2><p>TIME LIMITは異常を復旧・解明するまでの期限で、設備状態に関係なく減少します。本施設の基準値は8時間です。PLANT TIMEは現在の設備状態、進行中の異常、自動保護動作を含む因果関係から予測した物理的な余裕時間を示します。安全側へ収束して物理的な破綻が予測されない場合は --:--、物理的な余裕がTIME LIMITより長い場合は &gt; TIME LIMIT と表示されます。</p><h2>入力履歴</h2><p>ターミナル入力欄で↑／↓キーを使うと、過去に実行したコマンドを呼び出せます。</p>
  ...
  let current=0, host=null, insertCommand=null, eventHooks=null;
  ...
  function indexBody(){
    const sections=[
      ...
      ['一次系',['reactor-rcs','rcp','steam-generator','rcs-injection']],
      ['二次系／補助系',['main-feedwater','afw','main-steam','condenser']],
      ['安全系／電気系',['electrical','protection','instrumentation','ccw']],
  ...
  function coverBody(){
    return `<div class="cover"><h1>PLANT-01</h1><h2>加圧水型原子炉施設</h2><div class="subtitle">運転・保守マニュアル</div><div class="docno">DOCUMENT NO. PL01-OM-001<br>REVISION 07</div><div class="auth">関係者用</div></div>`;
  }
  ...
  function draw(){
    ...
    article.querySelectorAll('[data-command]').forEach(b=>b.onclick=()=>{insertCommand?.(b.dataset.command);eventHooks?.onCommand?.(b.dataset.command);});
    article.scrollTop=0;
    eventHooks?.onPage?.(page.id);
  }

  return {
    render(root,insert,startPage,hooks=null){host=root;insertCommand=insert;eventHooks=hooks;root.className='right-body manual-host';if(startPage&&PAGE_BY_ID[startPage])current=PAGE_BY_ID[startPage].index;draw();},
    goto:gotoPage,
    reset(){current=0;host=null;insertCommand=null;eventHooks=null;}
  };
})();
